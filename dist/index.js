...
const tempo_client_js_1 = require("./tempo-client.js");
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "Create a new worklog with role and account support. Account is automatically fetched from the issue if not provided.",
                inputSchema: {
                    type: "object",
                    properties: {
                        issueKey: {
                        ...
                        accountKey: {
                            type: "string",
                            description: "Account key (optional - will use issue's account if not provided)",
                        },
                    },
        ...
        switch (name) {
            case "tempo_get_worklogs": {
                ...
                const formatted = worklogs.map((w) => {
                    ...
                    const role = w.attributes?.values.find((a) => a.key.toLowerCase().includes("role") || a.key.toLowerCase().includes("rôle"))?.value || "N/A";
                    const account = w.attributes?.values.find((a) => a.key.toLowerCase().includes("account"))?.value || "N/A";
            ...
            case "tempo_get_roles": {
                ...
                const formatted = roles.map((r) => `ID: ${r.id} | Name: ${r.name} | Default: ${r.default}`);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "tempo_log_sprint_meeting": {
                const { projectKey, timeSpentMinutes, description, date } = args;
